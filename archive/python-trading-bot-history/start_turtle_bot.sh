...
if [ ! -f "turtle-reverse-bot/config/config.json" ]; then
    echo "❌ 配置文件不存在: turtle-reverse-bot/config/config.json"
    exit 1
fi
...
if grep -q "YOUR_" "turtle-reverse-bot/config/config.json"; then
    ...
    echo "📝 编辑文件: turtle-reverse-bot/config/config.json"
    echo ""
    echo "是否继续启动? (y/N)"
    read -r response
    if [[ ! "$response" =~ ^[Yy]$ ]]; then
...
# 创建日志目录
mkdir -p turtle-reverse-bot/logs
...
echo "策略: 分层滚仓止盈反向跟单"
echo "时间: $(date)"
echo "目录: $(pwd)/turtle-reverse-bot"
echo ""
